...
export default function InstructorManageAssignment({
    ...
    const handleAddQuestion = async () => {
        ...
        if (correctCount !== 1) {
            toast.warning("Select exactly one correct answer");
            return;
        }
        ...
        try {
            ...
            if (editingQuestion) {
                // UPDATE
                await fetch(
                    `http://localhost:3000/api/assignments/question/${editingQuestion.id}`,
                    {
                        method: "PUT",
                        headers: {
                ...
                await fetch(
                    `http://localhost:3000/api/assignments/question/${newQuestion.id}/options`,
                    {
                        method: "POST",
                        headers: {
                            "Content-Type": "application/json",
    ...
    const handleDeleteQuestion = async (questionId: number) => {
        await fetch(
            `http://localhost:3000/api/assignments/question/${questionId}`,
            {
                method: "DELETE",
                headers: {
                    Authorization: `Bearer ${localStorage.getItem("accessToken")}`,
                    ...
                    <div className="relative bg-white w-full max-w-md mx-4 rounded-2xl shadow-2xl p-6 animate-in fade-in zoom-in-95">
                        ...
                        <div className="flex justify-end gap-3">
                            ...
                            <Button
                                variant="destructive"
                                onClick={async () => {
                                    await handleDeleteQuestion(confirmDeleteId);

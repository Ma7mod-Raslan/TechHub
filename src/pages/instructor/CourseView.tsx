...
export default function InstructorCourseView({
  ...
  const fetchCourse = async () => {
    ...
    setCourse({ ...data, students: 0, rating: 0, revenue: 0 });
  };
    ...
    const normalizedVideos = Array.isArray(data)
      ...
      : Array.isArray(data.videos)
        ...
        : Array.isArray(data.data)
          ? data.data
          : [];
    setVideos(normalizedVideos.sort((a: Video, b: Video) => a.video_order - b.video_order));
  };
  ...
  useEffect(() => {
    if (!mcqError) return;
    const timer = setTimeout(() => setMcqError(''), 5000);
    return () => clearTimeout(timer);
  }, [mcqError]);

  const validateVideoForm = () => {
    const errors = { title: '', url: '', description: '' };
    let isValid = true;
    if (!videoFormData.title.trim()) { errors.title = 'Title is required'; isValid = false; }
    if (!videoFormData.url.trim()) { errors.url = 'URL is required'; isValid = false; }
    if (!videoFormData.description.trim()) { errors.description = 'Description is required'; isValid = false; }
  ...
  const handleEditVideo = async () => {
    ...
    if (!validateVideoForm()) return;
    const token = localStorage.getItem('accessToken');
    await fetch(`/api/courses/${courseId}/videos/${selectedVideo.id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
      body: JSON.stringify({
        title: videoFormData.title,
  ...
  const [formErrors, setFormErrors] = useState({
    title: '', description: '', videoTitle: '', videoUrl: '', videoDuration: '',
  });

  const getCategoryValue = (category: string) => category?.toLowerCase() ?? '';
  ...
  const handleCancelEdit = () => {
    ...
    setFormErrors({ title: '', description: '', videoTitle: '', videoUrl: '', videoDuration: '' });
    setIsEditMode(false);
  };
  ...
  const validateCourseForm = () => {
    let valid = true;
    const errors: any = {};
    if (!editFormData.title.trim()) { errors.title = 'Title is required'; valid = false; }
    if (!editFormData.description.trim()) { errors.description = 'Description is required'; valid = false; }
  ...
  const moveVideo = async (video: Video, direction: 'up' | 'down') => {
    const token = localStorage.getItem('accessToken');
    const targetOrder = direction === 'up' ? video.video_order - 1 : video.video_order + 1;
    if (targetOrder < 1 || targetOrder > videos.length) return;
    const targetVideo = videos.find(v => v.video_order === targetOrder);
    if (!targetVideo) return;
    await fetch(`/api/courses/${courseId}/videos/reorder`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
      body: JSON.stringify({ videoId: video.id, targetVideoId: targetVideo.id }),
    });
    await fetchVideos();
  };
  ...
  const handleAddQuestion = async () => {
    if (!selectedVideo) return;
    if (!questionText.trim()) { setMcqError('Question text is required'); return; }
    const correctCount = choices.filter(c => c.is_correct).length;
    if (correctCount !== 1) { setMcqError('Please select exactly one correct answer'); return; }
    setMcqError('');
    const token = localStorage.getItem('accessToken');
    const res = await fetch(`/api/videos/${selectedVideo.id}/questions`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
      body: JSON.stringify({ question_text: questionText, choices }),
    });
    if (!res.ok) {
    ...
    <div className="min-h-screen bg-gray-50">
          ...
          <header className="bg-white border-b px-4 md:px-6 py-4 sticky top-0 z-30">
            <div className="flex items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-2xl">{isEditMode ? 'Edit Course Details' : 'Course Details'}</h1>
                  <p className="text-gray-600">{isEditMode ? 'Update your course information' : 'View and manage your course'}</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3">
                ...
                {!isEditMode ? (
                  ...
                  <>
                    ...
                    <Button className="bg-gradient-to-r from-violet-600 to-cyan-500 hover:from-violet-700 hover:to-cyan-600" onClick={handleSaveCourse} disabled={isSaving}>
            ...
            <CardContent className="p-6">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="md:col-span-2 space-y-4">
                  ...
                  <p className="text-gray-600">{course.description || 'No description provided for this course.'}</p>
                </div>
              </div>
            </CardContent>
          </Card>
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between mb-6">
                  ...
                  <Button
                    ...
                    onClick={() => {
                      setSelectedVideo(null);
                      setVideoFormData({ title: '', url: '', description: '', order: videos.length + 1 });
                    ...
                    {videos.map((video, index) => (
                      ...
                      >
                        ...
                        <div className="flex gap-2">
                          <Button size="icon" variant="ghost" disabled={video.video_order === 1} onClick={() => moveVideo(video, 'up')} className="hover:bg-violet-50">
                            <ChevronUp className="h-4 w-4 text-gray-600" />
                          </Button>
                          <Button size="icon" variant="ghost" disabled={video.video_order === videos.length} onClick={() => moveVideo(video, 'down')} className="hover:bg-violet-50">
                          ...
                          <Button size="sm" variant="ghost" onClick={() => {
                            setSelectedVideo(video);
                            setVideoFormData({ title: video.title, url: video.video_url, description: video.description || '', order: video.video_order });
                            setShowEditVideoDialog(true);
                          }}>
                            <Edit className="h-4 w-4" />
                          </Button>
                          <Button size="sm" variant="ghost" onClick={() => { setSelectedVideo(video); setShowDeleteVideoDialog(true); }}>
      ...
      <Dialog open={showAddVideoDialog} onOpenChange={setShowAddVideoDialog}>
        <DialogContent>
          ...
          <Input placeholder="Title" value={videoFormData.title} onChange={e => { setVideoFormData({ ...videoFormData, title: e.target.value }); setVideoErrors({ ...videoErrors, title: '' }); }} />
          {videoErrors.title && <p className="text-sm text-red-600">{videoErrors.title}</p>}
          <Input placeholder="URL" value={videoFormData.url} onChange={e => { setVideoFormData({ ...videoFormData, url: e.target.value }); setVideoErrors({ ...videoErrors, url: '' }); }} />
          {videoErrors.url && <p className="text-sm text-red-600">{videoErrors.url}</p>}
          <Textarea placeholder="Description" value={videoFormData.description} onChange={e => { setVideoFormData({ ...videoFormData, description: e.target.value }); setVideoErrors({ ...videoErrors, description: '' }); }} />
          {videoErrors.description && <p className="text-sm text-red-600">{videoErrors.description}</p>}
          <DialogFooter>
      ...
      <Dialog open={showEditVideoDialog} onOpenChange={setShowEditVideoDialog}>
        <DialogContent>
          ...
          <Input placeholder="Title" value={videoFormData.title} onChange={e => { setVideoFormData({ ...videoFormData, title: e.target.value }); setVideoErrors({ ...videoErrors, title: '' }); }} />
          {videoErrors.title && <p className="text-sm text-red-600">{videoErrors.title}</p>}
          <Input placeholder="Youtube Video URL" value={videoFormData.url} onChange={e => { setVideoFormData({ ...videoFormData, url: e.target.value }); setVideoErrors({ ...videoErrors, url: '' }); }} />
          {videoErrors.url && <p className="text-sm text-red-600">{videoErrors.url}</p>}
          <Textarea placeholder="Description" value={videoFormData.description} onChange={e => { setVideoFormData({ ...videoFormData, description: e.target.value }); setVideoErrors({ ...videoErrors, description: '' }); }} />
          {videoErrors.description && <p className="text-sm text-red-600">{videoErrors.description}</p>}
          <DialogFooter>
      ...
      <Dialog open={showMCQDialog} onOpenChange={setShowMCQDialog}>
        <DialogContent className="max-h-[90vh] flex flex-col">
          <DialogHeader><DialogTitle>Add MCQ Question</DialogTitle></DialogHeader>
          <div className="flex-1 overflow-y-auto pr-2">
            ...
            <div className="space-y-3 mt-4">
              {choices.map((choice, index) => (
                <div key={index} className="flex items-center gap-2">
                  <input type="radio" name="correct-choice" checked={choice.is_correct} onChange={() => setChoices(choices.map((c, i) => ({ ...c, is_correct: i === index })))} />
                  <Input placeholder={`Choice ${index + 1}`} value={choice.text} onChange={e => setChoices(choices.map((c, i) => i === index ? { ...c, text: e.target.value } : c))} />
                  <Button type="button" variant="ghost" size="icon" disabled={choices.length <= 2} onClick={() => {
                    if (choices.length <= 2) return;
                    const newChoices = choices.filter((_, i) => i !== index);
                    if (choice.is_correct) newChoices.forEach(c => (c.is_correct = false));
            ...
            <Button variant="outline" className="mt-3" disabled={choices.length >= 4} onClick={() => { if (choices.length < 4) setChoices([...choices, { text: '', is_correct: false }]); }}>
              + Add Choice
            </Button>
            <div className="mt-6">
                  ...
                  {questions.map((q, index) => (
                    <div key={q.id} className="border rounded-lg p-3 bg-gray-50">
                      ...
                      <ul className="mt-2 space-y-1">
                        {q.choices.map((c: any) => (
                          <li key={c.id} className="text-sm text-gray-700">• {c.choice_text}</li>
                        ))}
                      </ul>
                    </div>
                  ))}
          ...
          <DialogFooter className="flex flex-col items-end gap-2 pt-3">
            {mcqError && (
              <div className="flex items-center gap-2 text-red-600 text-sm self-start">
                <AlertCircle className="h-4 w-4" />{mcqError}
              </div>
            )}

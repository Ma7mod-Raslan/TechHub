...
import { loginWithGoogle } from "../../firebase"; // عدّلي المسار لو عندك غيره
...
export default function SignUp({ navigate }: SignUpProps) {
  ...
  // ===== Google signup handler (sends role as hint to backend; fallback to client role) =====
  const handleGoogleSignup = async (): Promise<void> => {
    try {
      setLoading(true);
      const result = await loginWithGoogle();
      const user = result.user;
      if (!user) throw new Error("No Google user returned");

      const token = await user.getIdToken();

      // send to backend to register/login via Google
      const res = await api.post("/auth/google", { id_token: token, role });
      const data = res.data;

      // if backend returned auth token + user, save them
      if (data?.token) {
      ...
      // navigate using backend role when available, otherwise use selected role
      ...
      alert(error?.response?.data?.error || error?.message || "Google signup failed");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSignUp = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await api.post("/auth/signup", {
        ...
        role, // important: send selected role to backend
      });

      // keep pending verification context so Verification page knows email & role
      localStorage.setItem("pendingVerification", JSON.stringify({ email: formData.email, role }));

      alert(res.data.message || "Account created. Check email for verification code.");

      // navigate to verification page (component name: Verification / verification)
      // we pass role so the verification page can redirect after success
        ...
        <Card>
          <CardContent className="pt-6">
            <form onSubmit={handleSignUp} className="space-y-4">
              ...
              <div>
                ...
                <div className="relative mt-1">
                  ...
                  <Input id="name" type="text" placeholder="John Doe" className="pl-10" value={formData.name} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setFormData({ ...formData, name: e.target.value })} required />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="relative mt-1">
                  ...
                  <Input id="email" type="email" placeholder="you@example.com" className="pl-10" value={formData.email} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setFormData({ ...formData, email: e.target.value })} required />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="relative mt-1">
                  ...
                  <Input id="password" type={showPassword ? "text" : "password"} placeholder="Create a strong password" className="pl-10 pr-10" value={formData.password} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setFormData({ ...formData, password: e.target.value })} required />
            ...
            <p className="text-center text-sm text-gray-600 mt-6">
              ...
              <button onClick={() => navigate("login")} className="text-cyan-600 hover:text-cyan-700">
                Sign in
              </button>
            </p>
          </CardContent>
